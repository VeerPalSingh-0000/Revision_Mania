  ...
  const uniqueProblems = useMemo(() => {
    ...
    problems.forEach(problem => {
      ...
      if (problemMap.has(canonical)) {
        ...
        if (problem.tags && problem.tags.length > 0) existing.tags = problem.tags;
        ...
        <table className="min-w-full divide-y divide-slate-700">
          ...
          <tbody className="divide-y divide-slate-800 bg-slate-900/50">
            {uniqueProblems.map((problem, index) => (
              <tr key={problem.id || index} className="hover:bg-slate-800/50 group/row transition-colors">
                <td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-slate-500 sm:pl-6">{index + 1}</td>
                      ...
                      {activeTooltip === index && (
                        ...
                        >
                          ...
                          <div className="relative z-10 space-y-3">
                            ...
                            {/* Topics/Tags Info */}
                            <div>
                              <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider block mb-2">Topics</span>
                              <div className="flex flex-wrap gap-1.5">
                                {problem.tags && problem.tags.length > 0 ? (
                                  problem.tags.map((tag, i) => (
                                    <span key={i} className="text-[10px] px-2 py-1 rounded-full bg-slate-700 text-slate-300 border border-slate-600">
                                      {tag}
                                    </span>
                                  ))
                                ) : (
