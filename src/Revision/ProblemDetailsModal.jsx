...
const DifficultyBadge = ({ difficulty }) => {
  ...
  return (
    <span
      className={`px-3 py-1 rounded-full text-sm font-medium border ${getDifficultyColor(difficulty)}`}
    >
      {difficulty}
    </span>
  );
...
export default function ProblemDetailsModal({
  ...
  const nextDueDate = !isRevision
    ? getNextDueDate(lastSolved, problem.solveCount, intervals)
    : null;
  ...
  const handleNotesSave = () => {
    ...
    if (
      trimmed !== (problem.notes || "").trim() &&
      typeof onUpdateNotes === "function"
    ) {
      onUpdateNotes(problem.id, trimmed);
    }
  };
  ...
  const daysDiff = nextDueDate
    ? Math.ceil((nextDueDate - new Date()) / (1000 * 60 * 60 * 24))
    : 99;
  const dueVariant = (() => {
    if (daysDiff < 0) return "bg-red-600/20 text-red-300 border-red-600/30";
    if (daysDiff <= 1)
      ...
      >
        ...
        <div className="pr-10 mb-6">
          ...
          <h2 className="text-xl sm:text-2xl font-bold text-slate-100 leading-tight mb-3 break-words">
            {isLink ? (
              <a
                ...
                className="inline-flex items-center gap-3 hover:text-cyan-400 transition-colors break-words"
              >
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {problem.platform && (
              <span className="px-3 py-1 rounded-full text-sm font-medium text-cyan-300 bg-cyan-400/10 border border-cyan-400/20">
                {problem.platform}
              </span>
            )}
            <span className="ml-auto text-sm text-slate-400">
              Solved {problem.solveCount || 0} time
              {(problem.solveCount || 0) !== 1 ? "s" : ""}
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
          <div className="flex items-start gap-3 p-3 rounded-lg bg-slate-800/50 border border-slate-700/40">
            <div className="flex-shrink-0 flex h-10 w-10 items-center justify-center rounded-lg bg-emerald-500/10 text-emerald-300">
              <FiCheckSquare />
            </div>
            <div>
          ...
          {!isRevision && nextDueDate && (
            <div className="flex items-start gap-3 p-3 rounded-lg bg-slate-800/50 border border-slate-700/40">
              <div className="flex-shrink-0 flex h-10 w-10 items-center justify-center rounded-lg bg-cyan-500/10 text-cyan-300">
                <FiCalendar />
              </div>
              <div>
                ...
                <p
                  className={`text-xs mt-1 inline-block px-2 py-0.5 rounded-md font-medium ${dueVariant} border`}
                >
                  {getDaysUntilDue()}
                </p>
              </div>
          ...
          <div className="mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              {problem.tags.map((tag, index) => (
                <span
                  key={index}
                  className="px-3 py-1 text-xs rounded-md bg-slate-700/50 text-slate-300 border border-slate-600/50"
                >
                  #{tag}
                </span>
              ))}
        ...
        <div className="mb-6">
          ...
          {isEditingNotes ? (
            <textarea
              ...
              className="w-full rounded-lg border border-cyan-500/50 bg-slate-800/50 p-3 text-sm text-slate-200 placeholder:text-slate-500 focus:border-cyan-500 focus:outline-none focus:ring-1 focus:ring-cyan-500 resize-none"
            />
          ) : (
            <div
              onClick={() => setIsEditingNotes(true)}
              className="min-h-[48px] cursor-pointer rounded-lg border border-slate-700/40 bg-slate-800/30 p-3 text-sm transition-colors hover:border-slate-600/50 hover:bg-slate-800/50"
            >
              {notesValue ? (
        ...
        <div className="flex gap-3 pt-4 border-t border-slate-700/50">
          {!isRevision && (
            <button
              ...
              className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-3 rounded-lg bg-emerald-500/10 text-emerald-300 font-medium transition-colors hover:bg-emerald-500/20 border border-emerald-500/30 min-h-[48px]"
            >
              <FiRepeat />
              Solve Again
            </button>
