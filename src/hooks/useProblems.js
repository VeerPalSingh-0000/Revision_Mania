...
export function useProblems(user) {
  ...
  const markAsSolved = useCallback(async (problemId) => {
    try {
      ...
      await updateDoc(problemRef, {
        ...
        date: Timestamp.fromDate(new Date()) // Reset the revision date
      });
      return { success: true };
    } catch (err) {
  ...
  const addProblem = useCallback(async (problemData) => {
    ...
    const problemText = (typeof problemData === 'string' ? problemData.trim() : problemData.problem.trim());
    ...
    try {
      ...
      if (!querySnapshot.empty) {
        ...
        const newProblem = {
          ...
          solveCount: 0, // <-- CORRECTED: Start count at 0
          isRevision: false,
