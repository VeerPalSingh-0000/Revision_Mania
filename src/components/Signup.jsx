...
export default function SignUp({ onSwitchToSignIn }) {
    ...
    return (<div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-slate-950 p-4 font-sans"><div className="absolute inset-0 z-0 bg-[radial-gradient(#1e293b_1px,transparent_1px)] [background-size:24px_24px]"></div><motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }} className="relative z-10 w-full max-w-lg rounded-2xl border border-slate-800 bg-slate-900 p-8 md:p-12"><header className="text-center"><h1 className="mb-2 text-3xl font-bold tracking-tighter text-slate-100 md:text-4xl">Create an Account</h1><p className="text-sm text-slate-400">Join us and start your journey.</p></header><div className="mt-8 space-y-6"><form onSubmit={handleSubmit} className="space-y-6"><FormInput id="name" type="text" placeholder="Full Name" icon={FiUser} value={name} onChange={(e) => setName(e.target.value)} /><FormInput id="email" type="email" placeholder="Email" icon={FiMail} value={email} onChange={(e) => setEmail(e.target.value)} /><FormInput id="password" type="password" placeholder="Password" icon={FiLock} value={password} onChange={(e) => setPassword(e.target.value)} /><PasswordStrengthMeter strength={passwordStrength} /><ActionButton isLoading={isLoading}>Create Account</ActionButton></form><div className="my-4 flex items-center gap-3"><hr className="w-full border-slate-700" /><span className="text-xs font-medium text-slate-500">OR</span><hr className="w-full border-slate-700" /></div><SocialButton onClick={handleGoogleSignUp} disabled={isLoading}>Sign Up with Google</SocialButton><p className="text-center text-sm text-slate-400">Already have an account?{' '}<button onClick={onSwitchToSignIn} className="font-medium text-cyan-400 hover:underline">Sign In</button></p></div></motion.div></div>);
};
