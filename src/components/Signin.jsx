...
import { signIn, signInWithGoogle, resetPassword } from "./Auth.jsx";
...
export default function SignIn({ onSwitchToSignUp }) {
    ...
    const handleAction = async (action) => {
        ...
        // onSuccess is handled by the onAuthStateChanged listener in App.jsx
    };

    const handleSubmit = (e) => { e.preventDefault(); handleAction(() => signIn(email, password)); };
    const handleGoogleSignIn = () => handleAction(signInWithGoogle);
    const handlePasswordReset = () => {
        if (!email) { setStatus({ state: 'error', message: 'Enter your email to receive a reset link.' }); return; }
        resetPassword(email).then(({error}) => {
            if (error) {
                setStatus({ state: 'error', message: error });
            } else {
                setStatus({ state: 'success', message: 'Password reset link sent!' });
            }
        });
    };

    return (<div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-slate-950 p-4 font-sans"><div className="absolute inset-0 z-0 bg-[radial-gradient(#1e293b_1px,transparent_1px)] [background-size:24px_24px]"></div><motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }} className="relative z-10 w-full max-w-lg rounded-2xl border border-slate-800 bg-slate-900 p-8 md:p-12"><header className="text-center"><h1 className="mb-2 text-3xl font-bold tracking-tighter text-slate-100 md:text-4xl">Welcome Back</h1><p className="text-sm text-slate-400">Let's sign you in to continue.</p></header><div className="mt-8 space-y-6"><AnimatePresence>{(status.state === "error" || status.state === "success") && (<AlertMessage message={status.message} isError={status.state === "error"} />)}</AnimatePresence><form onSubmit={handleSubmit} className="space-y-6"><FormInput id="email" type="email" placeholder="Email" icon={FiMail} value={email} onChange={(e) => setEmail(e.target.value)} /><FormInput id="password" type="password" placeholder="Password" icon={FiLock} value={password} onChange={(e) => setPassword(e.target.value)} /><div className="flex justify-end"><button type="button" onClick={handlePasswordReset} className="text-sm font-medium text-cyan-400 transition-colors hover:text-cyan-300">Forgot Password?</button></div><ActionButton isLoading={status.state === 'loading'}>Sign In</ActionButton></form><div className="my-4 flex items-center gap-3"><hr className="w-full border-slate-700" /><span className="text-xs font-medium text-slate-500">OR</span><hr className="w-full border-slate-700" /></div><SocialButton onClick={handleGoogleSignIn} disabled={status.state === 'loading'}>Continue with Google</SocialButton><p className="text-center text-sm text-slate-400">Don't have an account?{' '}<button onClick={onSwitchToSignUp} className="font-medium text-cyan-400 hover:underline">Sign Up</button></p></div></motion.div></div>);
};
